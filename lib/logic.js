...
function addInsurer(tx) {
    var factory = getFactory();
    var NS="health"
    var asset = factory.newResource(NS,'InsuranceProvider',tx.insurer_details.insurenceProviderId);
...
function registerInsurance(tx) {
    ...
    medicAsset.Insurence_end_date = tx.Insurence_end_date;
 ...
 */
 ...
 function claimInsurance(tx){
    ...
    if(patient.MrnNumber != medicInsurance.patient.MrnNumber){
        throw new Error("Insurace specified does not belong to You")
    }
